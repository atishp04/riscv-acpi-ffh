...
=== Software Managed MSI Controller(SMMC) Object

ACPI describes the Software Managed MSI Controller (SMMC) that allows platform MSI
devices to manage their MSIs without a central irq to MSI bridge.
The ACPI _SMC object describes the mechanism for the operating system to map GSIs to MSIs
for platform MSI devices. The Fields in the _SMC object may be static integers or “Resource
Descriptors”.  The following subsection specifies a RISC-V system “Resource
Descriptor” for the _SMC object.

==== _SMC Object Resource Descriptor

The pass:[_]SMC object may use a “Resource Descriptor”, which is formatted per
<<resource_descriptor_encoding>>, for many of its fields.  When using an FFH
Resource Descriptor for a _SMC field, it must be formatted as specified in
<<table_smc_resource_descriptor>>:

[#table_smc_resource_descriptor]
._SMC Resource Descriptor
[cols="^1,^2", width=90%, align="center", options="header"]
|===
| Field                           | Value
...
| _Access Size_                   | 4 (QWord)
| _Register Address_              | As specified in the Bits[63:60],
                                    Bits[59:32], Bits[31:0] columns of
                                    <<table_smc_register_address>>
|===

[#table_smc_register_address]
._SMC Register Address
[cols="^1,^1,^1,^1,^1", width=100%, align="center", options="header"]
|===
| Bits[63:60] +
  (Type)
| Bits[59:32] | Bits[31:12] | Bits[11:0] | Description
| 0x1   | 0x000_0000 2+| SBI MPXY Attribute ID | SBI MPXY Attribute
|===

All other encodings for pass:[_]SMC Resource Descriptors with _Address Space_
set to _FFixedHW_ (0x7f) are reserved for future use.

<<smmc_examples>> provides examples for modifying the MSI configuration via both
System Memory and an SBI MPXY.
...
[appendix]
[#smmc_examples]
== _SMC Object Examples
[source, C]
----
Device (SMC0) {              // SMC DSDT object
    Name (_HID, "RSCV0005")  // _HID: Hardware ID
    Name (_UID, Zero)        // _UID: Unique
    Name (_GSB, 0x61)  // _GSB: Global System Interrupt Base
    Name (CFGN, Package (0x02)        {
        Package () {
            0x61, //GSI Number
            ResourceTemplate () {   //MSI ADDR LOW
                Register(FFixedHW, 64, 0, 0x1000_0000_0000_0006, QWord) //MSI_ADDR_LOW Attribute in SBI MPXY
            },
            ResourceTemplate () {   //MSI ADDR HIGH
                Register(FFixedHW, 64, 0, 0x1000_0000_0000_0007, QWord) //MSI_ADDR_HIGH Attribute in SBI MPXY
            },
            ResourceTemplate () {   //MSI DATA
                Register(FFixedHW, 64, 0, 0x1000_0000_0000_0008, QWord) //MSI_DATA Attribute in SBI MPXY
            },
            ResourceTemplate () {   //MSI Enable
                Register(FFixedHW, 64, 0, 0x1000_0000_0000_0005, QWord) //MSI_CONTROL Attribute in SBI MPXY
            },
        }
        Package() {
            0x62, //GSI Number
            ResourceTemplate () { //MSI ADDR LOW
                Register(SystemMemory, 32, 0, 0x10220000, DWORD)
            },
            ResourceTemplate () { //MSI ADDR HIGH
                Register(SystemMemory, 32, 0, 0x10220004, DWORD)
            },
            ResourceTemplate () { //MSI DATA
                Register(SystemMemory, 32, 0, 0x10220008, DWORD)
            },
            ResourceTemplate () { //MSI Enable
                Register(SystemMemory, 32, 0, 0x1022000C, DWORD)
            },
        }
    )
}
